...
<template>
  <header class="sticky top-0 z-50 border-b border-sand-200/70 bg-sand-50/90 backdrop-blur">
    <div class="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
      <NuxtLink :to="localePath('/')" class="font-display text-3xl font-semibold text-slate-700">
        {{ t('header.brandFirst') }} <span class="text-forest-700">{{ t('header.brandSecond') }}</span>
      </NuxtLink>
      ...
      <nav class="hidden items-center gap-6 lg:flex">
        ...
        <BaseButton :to="localePath('/rezervace')" class="ml-2">{{ t('nav.bookingCta') }}</BaseButton>
      </nav>
      ...
      <div class="flex items-center gap-2 lg:hidden">
        ...
        <button
          type="button"
          class="inline-flex h-10 w-10 items-center justify-center rounded-full text-slate-700"
    ...
    >
      <nav v-if="isOpen" class="border-t border-sand-200 bg-sand-50 px-4 pb-6 pt-2 lg:hidden">
        ...
        <BaseButton :to="localePath('/rezervace')" class="mt-3 w-full">{{ t('nav.bookingCta') }}</BaseButton>
      </nav>
    </transition>
  </header>
</template>
